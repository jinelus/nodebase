...
export const GeminiDialog: React.FC<GeminiDialogProps> = ({
  ...
  const { data: credentials, isLoading } = useGetCredentialsByType('GEMINI')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Configure settings for the Gemini model here.</DialogDescription>
        </DialogHeader>
                        ...
                        {credentials?.map((credential) => (
                          <SelectItem key={credential.id} value={credential.id}>
                            <div className="flex items-center gap-2">
                              <img src="/logos/gemini.svg" alt="Gemini" className="h-5 w-5" />
                              {credential.name}
                            </div>
                          </SelectItem>
                        ))}
